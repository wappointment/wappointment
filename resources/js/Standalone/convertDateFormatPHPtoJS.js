...
const replacements = {
    ...
    'G' : {hour: '2-digit', hour24:true}, //0..23	Hours (24 hour time)
    'h' : {hour: 'numeric', hour12:true},//01..12	Hours (12 hour time used with a A.)
    'H' : {hour: '2-digit', hour24:true},//00..23	Hours (24 hour time)
...
export default function(format, dateJS = false){
    
    let keysObj = Object.keys(replacements)
    let new_format_pattern = '' 
    let new_format_object = {}
    let browser_lang = browserLang()
    ...
    for (let i = 0; i < keysObj.length; i++) {
        ...
        if(dateJS){
            let formatedDate = dateJS.toLocaleString(browser_lang, replacements[keysObj[i]])
            if(formatedDate == 0){
                formatedDate = '00'
            }
            pattern_replace[i] = stringg === undefined? '': formatedDate
        }else{
