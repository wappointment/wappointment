...
WappoSwift_DependencyContainer::getInstance()
    ...
    ->asSharedInstanceOf('Wappointment\ClassConnect\EmailValidator')
    ...
    ->asSharedInstanceOf('WappoSwift_Mime_IdGenerator')
    ->withDependencies([
        'mime.idgenerator.idright',
    ])

    ->register('mime.message')
    ->asNewInstanceOf('WappoSwift_Mime_SimpleMessage')
    ->withDependencies([
        'mime.headerset',
    ...
    ->register('mime.part')
    ->asNewInstanceOf('WappoSwift_Mime_MimePart')
    ->withDependencies([
        'mime.headerset',
        'mime.textcontentencoder',
        'cache',
    ...
    ->asNewInstanceOf('WappoSwift_Mime_Attachment')
    ->withDependencies([
        'mime.headerset',
    ...
    ->asNewInstanceOf('WappoSwift_Mime_EmbeddedFile')
    ->withDependencies([
        'mime.headerset',
        'mime.base64contentencoder',
        'cache',
    ...
    ->asNewInstanceOf('WappoSwift_Mime_SimpleHeaderFactory')
    ->withDependencies([
    ...
    ->asNewInstanceOf('WappoSwift_Mime_SimpleHeaderSet')
    ->withDependencies(['mime.headerfactory', 'properties.charset'])

    ->register('mime.qpheaderencoder')
    ->asNewInstanceOf('WappoSwift_Mime_HeaderEncoder_QpHeaderEncoder')
    ->withDependencies(['mime.charstream'])

    ->register('mime.base64headerencoder')
    ->asNewInstanceOf('WappoSwift_Mime_HeaderEncoder_Base64HeaderEncoder')
    ->withDependencies(['mime.charstream'])

    ->register('mime.charstream')
    ->asNewInstanceOf('WappoSwift_CharacterStream_NgCharacterStream')
    ->withDependencies(['mime.characterreaderfactory', 'properties.charset'])

    ->register('mime.bytecanonicalizer')
    ->asSharedInstanceOf('WappoSwift_StreamFilters_ByteArrayReplacementFilter')
    ->addConstructorValue([[0x0D, 0x0A], [0x0D], [0x0A]])
    ->addConstructorValue([[0x0A], [0x0A], [0x0D, 0x0A]])

    ->register('mime.characterreaderfactory')
    ->asSharedInstanceOf('WappoSwift_CharacterReaderFactory_SimpleCharacterReaderFactory')

    ->register('mime.textcontentencoder')
    ->asAliasOf('mime.qpcontentencoder')

    ->register('mime.safeqpcontentencoder')
    ->asNewInstanceOf('WappoSwift_Mime_ContentEncoder_QpContentEncoder')
    ->withDependencies(['mime.charstream', 'mime.bytecanonicalizer'])

    ->register('mime.rawcontentencoder')
    ->asNewInstanceOf('WappoSwift_Mime_ContentEncoder_RawContentEncoder')

    ->register('mime.nativeqpcontentencoder')
    ->withDependencies(['properties.charset'])
    ->asNewInstanceOf('WappoSwift_Mime_ContentEncoder_NativeQpContentEncoder')

    ->register('mime.qpcontentencoder')
    ->asNewInstanceOf('WappoSwift_Mime_ContentEncoder_QpContentEncoderProxy')
    ...
    ->asNewInstanceOf('WappoSwift_Mime_ContentEncoder_PlainContentEncoder')
    ...
    ->asSharedInstanceOf('WappoSwift_Mime_ContentEncoder_Base64ContentEncoder')

    ->register('mime.rfc2231encoder')
    ->asNewInstanceOf('WappoSwift_Encoder_Rfc2231Encoder')
    ->withDependencies(['mime.charstream']);
