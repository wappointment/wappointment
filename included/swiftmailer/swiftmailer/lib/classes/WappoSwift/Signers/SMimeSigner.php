 ...
 */
class WappoSwift_Signers_SMimeSigner implements WappoSwift_Signers_BodySigner
{
    ...
    /**
     * @var WappoSwift_StreamFilters_StringReplacementFilterFactory
     */
    protected $replacementFactory;

    /**
     * @var WappoSwift_Mime_SimpleHeaderFactory
     */
    ...
    public function __construct($signCertificate = null, $signPrivateKey = null, $encryptCertificate = null)
    {
        ...
        $this->replacementFactory = WappoSwift_DependencyContainer::getInstance()
    ...
    /**
     * Change the WappoSwift_Message to apply the signing.
     *
     * @return $this
     */
     ...
     */
    protected function smimeSignMessage(WappoSwift_Message $message)
    {
        ...
        $messageStream = new WappoSwift_ByteStream_TemporaryFileByteStream();
        $signMessage->toByteStream($messageStream);
        $messageStream->commit();
        $signedMessageStream = new WappoSwift_ByteStream_TemporaryFileByteStream();
        ...
        if (!openssl_pkcs7_sign(
            ...
            throw new WappoSwift_IoException(sprintf('Failed to sign S/Mime message. Error: "%s".', openssl_error_string()));
        }
     ...
     */
    protected function smimeEncryptMessage(WappoSwift_Message $message)
    {
        ...
        $messageStream->commit();
        $encryptedMessageStream = new WappoSwift_ByteStream_TemporaryFileByteStream();
        ...
        if (!openssl_pkcs7_encrypt(
            ...
            throw new WappoSwift_IoException(sprintf('Failed to encrypt S/Mime message. Error: "%s".', openssl_error_string()));
        }
    ...
    protected function copyHeaders(
        WappoSwift_Message $fromMessage,
        WappoSwift_Message $toMessage,
        array $headers = []
    ) {
     ...
     */
    protected function copyHeader(WappoSwift_Message $fromMessage, WappoSwift_Message $toMessage, $headerName)
    {
        ...
        switch ($header->getFieldType()) {
            case WappoSwift_Mime_Header::TYPE_TEXT:
                $headers->addTextHeader($header->getFieldName(), $header->getValue());
                break;
            case WappoSwift_Mime_Header::TYPE_PARAMETERIZED:
     ...
     */
    protected function clearAllHeaders(WappoSwift_Message $message)
    {
    ...
    /**
     * Wraps a WappoSwift_Message in a message/rfc822 MIME part.
     *
     * @return WappoSwift_MimePart
     */
    protected function wrapMimeMessage(WappoSwift_Message $message)
    {
        ...
        $wrappedMessage = new WappoSwift_MimePart($messageStream, 'message/rfc822');
        $wrappedMessage->setEncoder(new WappoSwift_Mime_ContentEncoder_PlainContentEncoder('7bit'));

        return $wrappedMessage;
    }

    protected function parseSSLOutput(WappoSwift_InputByteStream $inputStream, WappoSwift_Message $message)
    {
        $messageStream = new WappoSwift_ByteStream_TemporaryFileByteStream();
    ...
    /**
     * Merges an OutputByteStream from OpenSSL to a WappoSwift_Message.
     */
    protected function streamToMime(WappoSwift_OutputByteStream $fromStream, WappoSwift_Message $message)
    {
        ...
        $message->setEncoder(new WappoSwift_Mime_ContentEncoder_NullContentEncoder($encoding));
     ...
     * @return array
     */
    protected function parseStream(WappoSwift_OutputByteStream $emailStream)
    {
        $bufferLength = 78;
        $headerData = '';
        $headerBodySeparator = "\r\n\r\n";
        ...
        $bodyStream = new WappoSwift_ByteStream_TemporaryFileByteStream();
    ...
    protected function copyFromOpenSSLOutput(WappoSwift_OutputByteStream $fromStream, WappoSwift_InputByteStream $toStream)
    {
        $bufferLength = 4096;
        $filteredStream = new WappoSwift_ByteStream_TemporaryFileByteStream();
