 ...
 *
 * @deprecated since SwiftMailer 6.1.0; use WappoSwift_Signers_DKIMSigner instead.
 */
class WappoSwift_Signers_OpenDKIMSigner extends WappoSwift_Signers_DKIMSigner
{
    ...
    public function __construct($privateKey, $domainName, $selector)
    {
        if (!extension_loaded('opendkim')) {
            throw new WappoSwift_SwiftException('php-opendkim extension not found');
        }
    ...
    {
        $header = new WappoSwift_Mime_Headers_OpenDKIMHeader('DKIM-Signature');
        $headerVal = $this->dkimHandler->getSignatureHeader();
        if (false === $headerVal || is_int($headerVal)) {
            throw new WappoSwift_SwiftException('OpenDKIM Error: '.$this->dkimHandler->getError());
        }
    ...
    public function setHeaders(WappoSwift_Mime_SimpleHeaderSet $headers)
    {
        ...
        if (!is_numeric($this->signatureTimestamp)) {
            ...
            if (!OpenDKIM::setOption(OpenDKIM::OPTS_FIXEDTIME, $this->signatureTimestamp)) {
                throw new WappoSwift_SwiftException('Unable to force signature timestamp ['.openssl_error_string().']');
            }
        }
