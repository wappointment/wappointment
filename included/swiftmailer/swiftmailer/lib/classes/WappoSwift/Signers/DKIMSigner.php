 ...
 */
class WappoSwift_Signers_DKIMSigner implements WappoSwift_Signers_HeaderSigner
{
    /**
     * PrivateKey.
     *
     ...
     *
     * @var WappoSwift_Mime_Headers_ParameterizedHeader
     */
     ...
     *
     * @throws WappoSwift_SwiftException
     *
     * @return $this
     */
        ...
        switch ($hash) {
            ...
            case 'rsa-sha256':
                ...
                if (!defined('OPENSSL_ALGO_SHA256')) {
                    throw new WappoSwift_SwiftException('Unable to set sha256 as it is not supported by OpenSSL.');
                }
                break;
            default:
                throw new WappoSwift_SwiftException('Unable to set the hash algorithm, must be one of rsa-sha1 or rsa-sha256 (%s given).', $hash);
        }
     ...
     * @param bool $debug
     *
     * @return WappoSwift_Signers_DKIMSigner
     */
    ...
    private function getEncryptedHash()
    {
        ...
        if (!$pkeyId) {
            throw new WappoSwift_SwiftException('Unable to load DKIM Private Key ['.openssl_error_string().']');
        }
        if (openssl_sign($this->headerCanonData, $signature, $pkeyId, $algorithm)) {
            return $signature;
        }
        throw new WappoSwift_SwiftException('Unable to sign DKIM Hash ['.openssl_error_string().']');
    }
}
