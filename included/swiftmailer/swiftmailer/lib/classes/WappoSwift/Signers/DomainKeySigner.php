 ...
 */
class WappoSwift_Signers_DomainKeySigner implements WappoSwift_Signers_HeaderSigner
{
    /**
     * PrivateKey.
     *
            ...
            switch ($string[$i]) {
                ...
                case "\n":
                    if ("\r" == $this->bodyCanonLastChar) {
                        ...
                        // Wooops Error
                        throw new WappoSwift_SwiftException('Invalid new line sequence in mail found \n without preceding \r');
                    }
                    break;
                case ' ':
                case "\t":
    ...
    private function getEncryptedHash()
    {
        ...
        if (!$pkeyId) {
            throw new WappoSwift_SwiftException('Unable to load DomainKey Private Key ['.openssl_error_string().']');
        }
        if (openssl_sign($this->canonData, $signature, $pkeyId, OPENSSL_ALGO_SHA1)) {
            return $signature;
        }
        throw new WappoSwift_SwiftException('Unable to sign DomainKey Hash  ['.openssl_error_string().']');
    }
}
