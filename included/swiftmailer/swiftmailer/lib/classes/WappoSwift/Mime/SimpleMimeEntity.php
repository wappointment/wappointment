 ...
 * @author Chris Corbyn
 */
class WappoSwift_Mime_SimpleMimeEntity implements WappoSwift_Mime_CharsetObserver, WappoSwift_Mime_EncodingObserver
{
     ...
     */
    public function __construct(WappoSwift_Mime_SimpleHeaderSet $headers, WappoSwift_Mime_ContentEncoder $encoder, WappoSwift_KeyCache $cache, WappoSwift_IdGenerator $idGenerator)
    {
    ...
    /**
     * Get the {@link WappoSwift_Mime_SimpleHeaderSet} for this entity.
     *
     * @return WappoSwift_Mime_SimpleHeaderSet
     */
     ...
     *
     * @return WappoSwift_Mime_SimpleMimeEntity[]
     */
     ...
     *
     * @param WappoSwift_Mime_SimpleMimeEntity[] $children
    ...
    {
        return $this->body instanceof WappoSwift_OutputByteStream ? $this->readStream($this->body) : $this->body;
    }

    /**
     * Set the body of this entity, either as a string, or as an instance of
     * {@link WappoSwift_OutputByteStream}.
     *
     * @param mixed  $body
     ...
     * @return $this
     */
    public function setEncoder(WappoSwift_Mime_ContentEncoder $encoder)
    {
     ...
     * has changed.
     */
    public function encoderChanged(WappoSwift_Mime_ContentEncoder $encoder)
    {
        $this->notifyEncoderChanged($encoder);
    }
    ...
    protected function bodyToString()
    {
        ...
        if (isset($this->body) && empty($this->immediateChildren)) {
            if ($this->cache->hasKey($this->cacheKey, 'body')) {
                ...
                $this->cache->setString($this->cacheKey, 'body', $body, WappoSwift_KeyCache::MODE_WRITE);
            }
            $string .= $body;
        }
    ...
    /**
     * Write this entire entity to a {@see WappoSwift_InputByteStream}.
     */
    ...
    /**
     * Write this entire entity to a {@link WappoSwift_InputByteStream}.
     */
    protected function bodyToByteStream(WappoSwift_InputByteStream $is)
    {
        if (empty($this->immediateChildren)) {
            if (isset($this->body)) {
                if ($this->cache->hasKey($this->cacheKey, 'body')) {
                    ...
                    if ($this->body instanceof WappoSwift_OutputByteStream) {
    ...
    private function readStream(WappoSwift_OutputByteStream $os)
    {
        $string = '';
    ...
    private function assertValidBoundary($boundary)
    {
        if (!preg_match('/^[a-z0-9\'\(\)\+_\-,\.\/:=\?\ ]{0,69}[a-z0-9\'\(\)\+_\-,\.\/:=\?]$/Di', $boundary)) {
            throw new WappoSwift_RfcComplianceException('Mime boundary set is not RFC 2046 compliant.');
        }
    }
    ...
    private function notifyEncoderChanged(WappoSwift_Mime_ContentEncoder $encoder)
    {
        foreach ($this->immediateChildren as $child) {
    ...
    {
        if ($this->cache instanceof WappoSwift_KeyCache) {
