 ...
 * WappoSwift_AddressEncoder_Utf8AddressEncoder together with
 * WappoSwift_Transport_Esmtp_SmtpUtf8Handler. Your outbound SMTP server must support
 * the SMTPUTF8 extension.
 *
 * @author Christian Schmidt
 */
class WappoSwift_AddressEncoder_IdnAddressEncoder implements WappoSwift_AddressEncoder
{
    /**
     * Encodes the domain part of an address using IDN.
     *
     * @throws WappoSwift_AddressEncoderException If local-part contains non-ASCII characters
     */
    public function encodeString(string $address): string
    {
        ...
        if (false !== $i) {
            ...
            if (preg_match('/[^\x00-\x7F]/', $local)) {
                throw new WappoSwift_AddressEncoderException('Non-ASCII characters not supported in local-part', $address);
            }
    ...
    protected function idnToAscii(string $string): string
    {
        ...
        throw new WappoSwift_SwiftException('No IDN encoder found (install the intl extension or the true/punycode package');
    }
}
