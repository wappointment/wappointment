 ...
 * @author Chris Corbyn
 */
class WappoSwift_KeyCache_DiskKeyCache implements WappoSwift_KeyCache
{
     ...
     *
     * @var WappoSwift_KeyCache_KeyCacheInputStream
     */
    private $stream;
     ...
     */
    public function __construct(WappoSwift_KeyCache_KeyCacheInputStream $stream, $path)
    {
        $this->stream = $stream;
        $this->path = $path;
    }
    ...
    public function setString($nsKey, $itemKey, $string, $mode)
    {
        ...
        switch ($mode) {
            ...
            case self::MODE_APPEND:
                ...
                break;
            default:
                throw new WappoSwift_SwiftException(
     ...
     * @throws WappoSwift_IoException
     */
    public function importFromByteStream($nsKey, $itemKey, WappoSwift_OutputByteStream $os, $mode)
    {
        $this->prepareCache($nsKey);
        switch ($mode) {
            case self::MODE_WRITE:
     ...
     * @param string $itemKey
     *
     * @return WappoSwift_InputByteStream
     */
    public function getInputByteStream($nsKey, $itemKey, WappoSwift_InputByteStream $writeThrough = null)
    {
     ...
     * @param string                $nsKey
     * @param string                $itemKey
     * @param WappoSwift_InputByteStream $is      to write the data to
     */
    public function exportToByteStream($nsKey, $itemKey, WappoSwift_InputByteStream $is)
    {
        if ($this->hasKey($nsKey, $itemKey)) {
            $fp = $this->getHandle($nsKey, $itemKey, self::POSITION_START);
            while (!feof($fp) && false !== $bytes = fread($fp, 8192)) {
    ...
    private function prepareCache($nsKey)
    {
        ...
        if (!is_dir($cacheDir)) {
            if (!mkdir($cacheDir)) {
                throw new WappoSwift_IoException('Failed to create cache directory '.$cacheDir);
            }
            $this->keys[$nsKey] = [];
        }
    }
