 ...
 * @author Chris Corbyn
 */
abstract class WappoSwift_Transport_AbstractSmtpTransport implements WappoSwift_Transport
{
     ...
     */
    public function __construct(WappoSwift_Transport_IoBuffer $buf, WappoSwift_Events_EventDispatcher $dispatcher, $localDomain = '127.0.0.1', WappoSwift_AddressEncoder $addressEncoder = null)
    {
        $this->buffer = $buf;
    ...
    public function setAddressEncoder(WappoSwift_AddressEncoder $addressEncoder)
    {
    ...
    public function start()
    {
        if (!$this->started) {
            ...
            } catch (WappoSwift_TransportException $e) {
    ...
    public function send(WappoSwift_Mime_SimpleMessage $message, &$failedRecipients = null)
    {
        ...
        if (!$reversePath = $this->getReversePath($message)) {
            $this->throwException(new WappoSwift_TransportException(
        ...
        if ($evt) {
            if ($sent == count($to) + count($cc) + count($bcc)) {
                $evt->setResult(WappoSwift_Events_SendEvent::RESULT_SUCCESS);
            } elseif ($sent > 0) {
                $evt->setResult(WappoSwift_Events_SendEvent::RESULT_TENTATIVE);
            } else {
                $evt->setResult(WappoSwift_Events_SendEvent::RESULT_FAILED);
            }
     ...
     *
     * @return WappoSwift_Transport_IoBuffer
     */
    ...
    protected function streamMessage(WappoSwift_Mime_SimpleMessage $message)
    {
    ...
    protected function getReversePath(WappoSwift_Mime_SimpleMessage $message)
    {
    ...
    protected function throwException(WappoSwift_TransportException $e)
    {
    ...
    protected function assertResponseCode($response, $wanted)
    {
        ...
        if (!$valid) {
            $this->throwException(
                new WappoSwift_TransportException(
    ...
    protected function getFullResponse($seq)
    {
        ...
        try {
            ...
            $this->throwException($e);
        } catch (WappoSwift_IoException $e) {
            $this->throwException(
                new WappoSwift_TransportException(
                    $e->getMessage())
                );
    ...
    private function doMailTransaction($message, $reversePath, array $recipients, array &$failedRecipients)
    {
        ...
        foreach ($recipients as $forwardPath) {
            ...
            } catch (WappoSwift_AddressEncoderException $e) {
                $failedRecipients[] = $forwardPath;
            }
        }
    ...
    private function sendTo(WappoSwift_Mime_SimpleMessage $message, $reversePath, array $to, array &$failedRecipients)
    {
        if (empty($to)) {
            return 0;
        }
    ...
    private function sendBcc(WappoSwift_Mime_SimpleMessage $message, $reversePath, array $bcc, array &$failedRecipients)
    {
        $sent = 0;
