 ...
 * @author     Chris Corbyn
 */
class WappoSwift_ByteStream_FileByteStream extends WappoSwift_ByteStream_AbstractFilterableInputStream implements WappoSwift_FileStream
{
    ...
    {
        if (empty($path)) {
            throw new WappoSwift_IoException('The path cannot be empty');
        }
        $this->path = $path;
    ...
    private function getReadHandle()
    {
        if (!isset($this->reader)) {
            ...
            if (!$pointer) {
                throw new WappoSwift_IoException('Unable to open file for reading ['.$this->path.']');
            }
            $this->reader = $pointer;
            if (0 != $this->offset) {
    ...
    private function getWriteHandle()
    {
        if (!isset($this->writer)) {
            if (!$this->writer = fopen($this->path, $this->mode)) {
                throw new WappoSwift_IoException(
    ...
    private function copyReadStream()
    {
        if ($tmpFile = fopen('php://temp/maxmemory:4096', 'w+b')) {
            ...
            throw new WappoSwift_IoException('Unable to copy the file to make it seekable, sys_temp_dir is not writable, php://memory not available');
        }
        $currentPos = ftell($this->reader);
        fclose($this->reader);
        $source = fopen($this->path, 'rb');
        if (!$source) {
            throw new WappoSwift_IoException('Unable to open file for copying ['.$this->path.']');
        }
