 ...
 * @author Chris Corbyn
 */
class WappoSwift_StreamFilters_StringReplacementFilterFactory implements WappoSwift_ReplacementFilterFactory
{
    /** Lazy-loaded filters */
    private $filters = [];
     ...
     *
     * @return WappoSwift_StreamFilter
     */
    public function createFilter($search, $replace)
    {
        if (!isset($this->filters[$search][$replace])) {
            ...
            $this->filters[$search][$replace] = new WappoSwift_StreamFilters_StringReplacementFilter($search, $replace);
        }
